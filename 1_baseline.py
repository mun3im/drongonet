#!/usr/bin/env python3
"""
Baseline CNN-Mel Model (No modifications)
Pure baseline for comparison with ablation variants
...
def build_cnn_mel_model_table2(
    ...
    """
    Pure Baseline CNN-Mel Model
    No modifications - standard architecture from Table II
    """
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_baseline_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/1_baseline_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
    ...
    logger.info("="*60)
    logger.info("MyBAD CNN-Mel Baseline Training")
    logger.info("="*60)
