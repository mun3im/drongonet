#!/usr/bin/env python3
"""
XiaoChirp-Tiny: Compact CNN-Mel Model (96-width mel spectrogram)
Ultra-compact version with 96 time steps for minimal memory footprint
with aggressive GAP and depthwise
...
class TrainingConfig:
    ...
    n_mels: int = 80  # CNN-Mel original filter size
    n_fft: int = 1024  # FFT window size, fixed
    hop_length: int = 494  # Hop length for STFT (produces exactly 96 time steps)
    ...
    output_dir: str = 'results/13_tiny'
    cache_dir: str = '/Volumes/Evo/cache_mybad_tiny96'
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train XiaoChirp-Tiny CNN-Mel model (96-width)')
...
def build_cnn_mel_model_table2(
    ...
    """
    XiaoChirp-Tiny Model
    96-width mel spectrogram - ultra-compact version from Table II
    """
    ...
    # Block 2: aggressive depthwise-separable
    ...
    # Ultra-light head – no extra Dense
    x = tf.keras.layers.GlobalAveragePooling2D()(x)
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name=f"XiaoChirp_Tiny_GAP_Depthwise_Aggresssive_m{input_shape[1]}")
...
def compute_mel_spectrogram(waveform: np.ndarray, config: TrainingConfig) -> np.ndarray:
    ...
    # Ensure exactly 96 time steps
    if mel_spec_db.shape[0] > 96:
        mel_spec_db = mel_spec_db[:96, :]
    elif mel_spec_db.shape[0] < 96:
        pad_width = ((0, 96 - mel_spec_db.shape[0]), (0, 0))
...
class ModelTrainer:
    def __init__(self, model: tf.keras.Model, config: TrainingConfig, class_weights: Dict[int, float] = None):
        ...
        # Compile with standard categorical cross-entropy for balanced dataset
        # FIXED: Using categorical crossentropy instead of focal loss to prevent model collapse
        self.model.compile(
...
def save_config(config: TrainingConfig, output_dir: Path, args, system_info: dict):
    ...
    with open(config_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("TRAINING CONFIGURATION - XIAOCHIRP-TINY\n")
        f.write("="*60 + "\n\n")
        ...
        f.write(f"  Model: XiaoChirp-Tiny (96-width CNN-Mel)\n")
        f.write(f"  Input Shape: (96, {config.n_mels}, 1)\n")
        f.write("\n")
...
def main():
    ...
    # Calculate hop_length for exactly 96 time steps
    # Formula: hop_length = (target_length - n_fft) // (target_frames - 1)
    target_frames = 96
    config.hop_length = (config.target_length - config.n_fft) // (target_frames - 1)

    config.cache_dir = f'/Volumes/Evo/cache_mybad_tiny96_fft{config.n_fft}_m{config.n_mels}'
    config.output_dir = f'results/13_tiny_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
    ...
    logger.info("="*60)
    logger.info("XiaoChirp-Tiny CNN-Mel Training (96-width)")
    logger.info("="*60)
    ...
    try:
        ...
        # For balanced datasets, no class weights needed
        class_weights = None
        ...
        model = build_cnn_mel_model_table2(input_shape=(96, config.n_mels, 1), num_classes=2)
        ...
        with open(summary_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("XiaoChirp-Tiny Training Results Summary\n")
            f.write("="*60 + "\n\n")
            ...
            f.write(f"  (Experimental) Total Params: {len(tflite_model):,}\n")
