...
class TrainingConfig:
    ...
    def update_derived_paths(self):
        ...
        self.output_dir = f'{RESULTS_BASE}/3f_gap_focal_loss_freq_emph_pointwise_fft{self.n_fft}_m{self.n_mels}_s{self.random_seed}_{self.platform_tag}'
...
def build_model(input_shape=(184, 64, 1), num_classes=2):
    ...
    - Focal loss (applied during compilation)
    """
    ...
    return tf.keras.Model(inputs, outputs, name="DrongoNet_3f")
...
class ModelTrainer:
    def __init__(self, model, config, class_weights=None):
        ...
        self.model.compile(optimizer=get_optimizer(config.learning_rate), loss=focal_loss(gamma=2.0, alpha=0.5),
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("SEABAD CNN-Mel Training - GAP + Focal Loss + Freq Emphasis + Pointwise")
    logger.info("=" * 60)
