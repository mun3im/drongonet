#!/usr/bin/env python3
"""
4e_depthwise_drop01.py: Depthwise sep + Dropout(0.1) — Micro track (Phase 4B ablation)
SeparableConv2D, 4 filters, n_mels=16, GAP, focal loss + dropout=0.1.
...
def build_cnn_mel_model_table2(
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"SEABAD_depthwise_d01_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'{RESULTS_BASE}/4e_depthwise_drop01_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}_{platform_tag}'
