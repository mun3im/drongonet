...
class TrainingConfig:
    ...
    output_dir: str = 'results/2b_baseline_gap_learned'
...
def build_cnn(
    ...
    """
    Pure Baseline CNN-Mel Model with Global Average Pooling (GAP)
    + Learned spatial pooling (via soft attention) before GAP.
    """
    ...
    x = tf.keras.layers.Activation("relu")(x)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)  # Shape: (None, 44, 18, 4)

    # --- Learned Spatial Pooling (Soft Attention) ---
    # Use a 1x1 conv to produce attention logits (one per spatial location)
    attention_logits = tf.keras.layers.Conv2D(
        filters=1,
        kernel_size=1,
        activation=None,
        use_bias=False,
        name="attention_conv"
    )(x)  # Shape: (None, 44, 18, 1)

    # Flatten spatial dims, apply softmax, reshape back
    batch_size = tf.shape(x)[0]
    h, w = tf.shape(x)[1], tf.shape(x)[2]
    flat_logits = tf.reshape(attention_logits, [batch_size, -1])  # (B, H*W)
    attention_weights = tf.nn.softmax(flat_logits, axis=-1)       # (B, H*W)
    attention_weights = tf.reshape(attention_weights, [batch_size, h, w, 1])  # (B, H, W, 1)

    # Apply attention weights (broadcast over channels)
    x_weighted = x * attention_weights  # (B, H, W, C)

    # Now apply Global Average Pooling (weighted average)
    x_pooled = tf.keras.layers.GlobalAveragePooling2D()(x_weighted)

    # Final Dense layer
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax", name="classifier")(x_pooled)

    model = tf.keras.models.Model(inputs, outputs, name=f"SEABAD_baseline_GAP_learned_m{input_shape[1]}")
    return model
...
def save_config(config: TrainingConfig, output_dir: Path, args, system_info: dict):
    ...
    with open(config_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("TRAINING CONFIGURATION - MYBAD CNN-MEL\n")
        f.write("="*60 + "\n\n")
...
def main():
    ...
    config.output_dir = f'{RESULTS_BASE}/2b_baseline_gap_learned_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}_{platform_tag}'
    ...
    logger.info("="*60)
    logger.info("SEABAD CNN-Mel Baseline Training")
    logger.info("="*60)
    ...
    try:
        ...
        model = build_cnn(input_shape=(184, config.n_mels, 1), num_classes=2)
        ...
        with open(summary_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("SEABAD CNN-Mel Training Results Summary\n")
            f.write("="*60 + "\n\n")
