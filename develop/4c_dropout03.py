#!/usr/bin/env python3
"""
4c_dropout03.py: Dropout(0.3) sweep — Edge track (Phase 4A ablation)
Conv2D, 8 filters, gated n_mels, GAP, focal loss + dropout=0.3.
...
class TrainingConfig:
    ...
    output_dir: str = 'results_4c_dropout'
...
def build_cnn_mel_model_table2(
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"SEABAD_3a_a_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'{RESULTS_BASE}/4c_dropout03_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}_{platform_tag}'
