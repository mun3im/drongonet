...
class TrainingConfig:
    ...
    output_dir: str = 'results/3i_strided_focal_depthwise'
    ...
    def update_derived_paths(self):
        ...
        self.output_dir = f'{RESULTS_BASE}/3i_strided_focal_depthwise_fft{self.n_fft}_m{self.n_mels}_s{self.random_seed}_{self.platform_tag}'
...
def build_model(input_shape=(184, 64, 1), num_classes=2):
    """
    Build model with:
    - Frequency Emphasis layer
    - Strided Conv2D (no MaxPool) - TIER-1 optimization
    - Depthwise Separable Conv - TIER-2 optimization for efficiency
    ...
    # Depthwise separable convolution (TIER-2 optimization)
    x = tf.keras.layers.SeparableConv2D(16, (3, 3), padding='same', activation='relu')(x)

    # Global Average Pooling (removed redundant 1x1 conv)
    x = tf.keras.layers.GlobalAveragePooling2D()(x)

    # Output layer
    outputs = tf.keras.layers.Dense(num_classes, activation='softmax')(x)

    return tf.keras.Model(inputs, outputs, name="DrongoNet_3i")
...
def verify_model_features(model):
    ...
    has_separable = any('separable' in layer.__class__.__name__.lower() for layer in model.layers)
    logger.info(f"✓ Depthwise Separable Conv: {'PRESENT' if has_separable else 'MISSING'}")
    assert has_separable, "Model must have SeparableConv2D layer"
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("7g: Strided Conv + Focal Loss + Depthwise Separable")
    logger.info("=" * 60)
