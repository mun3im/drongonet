...
def build_cnn_time_model_gap_deeper(
    input_shape=(48000, 1),
    num_classes=2,
    ...
    """
    DEEPER 1D CNN for raw audio waveform classification.
    Adds one additional convolutional layer compared to 5b version.
    Uses GlobalAveragePooling1D to eliminate massive dense layers.
    Designed for ultra-low memory (<15 KB INT8) and fast inference on Cortex-M7.

    Architecture:
      - Conv1D(4, k=3) → ReLU → MaxPool(2)           # 48000 → 24000
      - Conv1D(8, k=3) → ReLU → AvgPool(2)           # 24000 → 12000
      - [Optional] Conv1D(16, k=9, dilation=2) → ReLU  # receptive enhancer
      - Conv1D(24, k=5) → ReLU → AvgPool(2)          # ← NEW DEEPER LAYER: 12000 → 6000
    ...
    if use_receptive_enhancer:
        ...
        x = layers.ReLU(name="relu3")(x)  # (12000, 16)

    # ========== NEW DEEPER LAYER ==========
    # Block 4: Additional depth for better feature extraction
    # Increases filters and adds another pooling stage
    current_filters = 16 if use_receptive_enhancer else 8
    x = layers.Conv1D(
        filters=24,
        kernel_size=5,
        padding="same",
    ...
    x = layers.AveragePooling1D(pool_size=2, name="avgpool2")(x)  # (6000, 24)
    # ======================================
    ...
    if use_pointwise_before_gap:
        ...
        x = layers.ReLU(name="relu_pointwise")(x)  # (6000, 32)

    # GLOBAL AVERAGE POOLING: replaces Flatten()
    x = layers.GlobalAveragePooling1D(name="gap")(x)  # (32,) if pointwise, else (24,)
    ...
    name_parts = ["TinyChirp_CNNTime_GAP_Deeper"]
    if use_receptive_enhancer:
...
def save_config(config: TrainingConfig, output_dir: Path, args, system_info: dict):
    ...
    with open(config_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("TRAINING CONFIGURATION - MYBAD CNN-TIME DEEPER GATEKEEPER\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"  Model: CNN-Time DEEPER with GAP\n")
        ...
        f.write(f"  Additional Layer: Conv1D(24, k=5) + AvgPool(2)\n")
        f.write("\n")
...
def main():
    ...
    config.output_dir = f'{RESULTS_BASE}/1f_cnntime_deeper{arch_suffix}_r{config.random_seed}_{platform_tag}'
    ...
    logger.info("=" * 60)
    logger.info("DrongoNet CNN-Time Deeper Reference Training")
    logger.info("=" * 60)
    ...
    try:
        ...
        # Build DEEPER model with architecture options
        logger.info(f"Architecture options: receptive_enhancer={args.use_receptive_enhancer}, pointwise={args.use_pointwise}")
        logger.info("Building DEEPER model with additional Conv1D(24, k=5) + AvgPool(2) layer")
        model = build_cnn_time_model_gap_deeper(
            input_shape=(48000, 1),
            num_classes=2,
        ...
        with open(summary_path, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MYBAD CNN-TIME DEEPER GATEKEEPER MODEL - RESULTS SUMMARY\n")
            f.write("=" * 80 + "\n\n")
