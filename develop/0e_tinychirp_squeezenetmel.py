...
class TrainingConfig:
    ...
    n_fft: int = 1024  # FFT window size fixed to paper
    hop_length: int = 256  # Hop length for STFT (adjusted to produce 184 time steps)
    ...
    output_dir: str = 'results_0e_tinychirp_squeezemel'
...
def fire_module_2d(x, squeeze_filters, expand_filters):
    """
    Fire module for 2D convolutions:
    - Squeeze: Conv2D with fewer filters
    - Expand: parallel Conv2D with 1x1 and 3x3 kernels
    """
    # Squeeze
    squeeze = tf.keras.layers.Conv2D(squeeze_filters, kernel_size=(1, 1), padding="same", activation="relu")(x)

    # Expand 1x1
    expand1x1 = tf.keras.layers.Conv2D(expand_filters, kernel_size=(1, 1), padding="same", activation="relu")(squeeze)

    # Expand 3x3
    expand3x3 = tf.keras.layers.Conv2D(expand_filters, kernel_size=(3, 3), padding="same", activation="relu")(squeeze)

    # Concatenate expand outputs
    return tf.keras.layers.Concatenate()([expand1x1, expand3x3])


def build_squeezenet_mel_model(
    input_shape=(184, 80, 1),
    num_classes=2
):
    """
    SqueezeNet-Mel model (Table II specification).

    Layers:
      - Conv2D(3x3, filters=8, same) + ReLU -> (184, 80, 8)
      - MaxPooling2D(2x2)                   -> (92, 40, 8)
      - Fire module (squeeze=4, expand=8)   -> (92, 40, 16)
      - Dropout(0.25)                       -> (92, 40, 16)
      - MaxPooling2D(2x2)                   -> (46, 20, 16)
      - Flatten                             -> 14720
      - Dense(8) + ReLU
    ...
    # Initial Conv
    x = tf.keras.layers.Conv2D(filters=8, kernel_size=(3, 3), padding="same", activation="relu")(inputs)  # (184, 80, 8)

    # MaxPool
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)  # (92, 40, 8)

    # Fire module
    x = fire_module_2d(x, squeeze_filters=4, expand_filters=8)  # (92, 40, 16)

    # Dropout
    x = tf.keras.layers.Dropout(0.25)(x)

    # MaxPool
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)  # (46, 20, 16)

    # Flatten
    x = tf.keras.layers.Flatten()(x)  # 46*20*16 = 14720
    ...
    model = tf.keras.models.Model(inputs, outputs, name="TinyChirp_SqueezeNetMel")
...
def main():
    ...
    config.output_dir = f'{RESULTS_BASE}/0e_tinychirp_squeezenetmel_r{config.random_seed}_{platform.system().lower()}'
    ...
    logger.info("="*60)
    logger.info("TinyChirp SqueezeNet-Mel Training")
    logger.info("="*60)
    ...
    try:
        ...
        model = build_squeezenet_mel_model(input_shape=(184, 80, 1), num_classes=2)
        ...
        with open(summary_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("TinyChirp SqueezeNet-Mel Training Results Summary\n")
            f.write("="*60 + "\n\n")
