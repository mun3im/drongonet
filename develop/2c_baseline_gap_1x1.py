...
class TrainingConfig:
    ...
    output_dir: str = 'results/2c_baseline_gap_1x1'
...
def build_cnn(
    ...
    x = tf.keras.layers.Conv2D(4, (1, 1), activation="relu", name="channel_mix")(x)
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"SEABAD_baseline_GAP_1x1_m{input_shape[1]}")
    return model
...
def main():
    ...
    config.output_dir = f'{RESULTS_BASE}/2c_baseline_gap_1x1_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}_{platform_tag}'
