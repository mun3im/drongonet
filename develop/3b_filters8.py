...
def build_cnn_mel_model_table2(
    ...
    """
    8-Filter Ablation Model
    Doubles filter count from 4 to 8 for capacity increase
    """
    inputs = tf.keras.layers.Input(shape=input_shape)

    # Block 1: 3x3 valid conv -> ReLU (ABLATION: 8 filters instead of 4)
    ...
    # Block 2: 3x3 valid conv -> ReLU (ABLATION: 8 filters instead of 4)
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"SEABAD_filters8_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'{RESULTS_BASE}/3b_filters8_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}_{platform_tag}'
    ...
    logger.info("="*60)
    logger.info("SEABAD 8-Filter Ablation Training")
    logger.info("="*60)
