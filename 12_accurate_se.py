#!/usr/bin/env python3
"""
XiaoChirp-Accurate with Squeeze-Excitation Block
Adds extra conv layer + SE block for channel attention
...
class TrainingConfig:
    ...
    n_mels: int = 64  # Optimal from V3 baseline sweep
    ...
    output_dir: str = 'results/12_accurate_se'
    cache_dir: str = '/Volumes/Evo/cache_mybad_m64'
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train XiaoChirp-Accurate with SE block')
...
def build_xiaochirp_accurate_se(
    input_shape=(184, 64, 1),
    num_classes=2
):
    """
    XiaoChirp-Accurate with Squeeze-Excitation
    Baseline architecture + extra conv layer + SE block for channel attention
    Target: >94.27% (beat baseline), aim for 95%+
    """
    inputs = tf.keras.layers.Input(shape=input_shape)

    # Block 1: 3x3 valid conv -> ReLU -> MaxPool
    x = tf.keras.layers.Conv2D(filters=4, kernel_size=(3, 3), padding="valid")(inputs)
    x = tf.keras.layers.Activation("relu")(x)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)

    # Block 2: NEW - Additional conv layer for increased capacity
    x = tf.keras.layers.Conv2D(filters=4, kernel_size=(3, 3), padding="same")(x)
    x = tf.keras.layers.Activation("relu")(x)

    # NEW - Squeeze-Excitation block for channel attention
    ...
    # Dropout for regularization (baseline uses 0.2 implicitly in some variants)
    x = tf.keras.layers.Dropout(0.2)(x)

    # FC + Softmax -> 2
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name=f"XiaoChirp_Accurate_SE_m{input_shape[1]}")
    ...
    def evaluate_tflite(self, tflite_path: Path, test_dataset: tf.data.Dataset) -> Tuple[float, float, float]:
        ...
        for inputs, labels in tqdm(test_dataset, desc="TFLite inference"):
            ...
            for i in range(batch_size):
                input_data = inputs_quantized[i:i + 1]  # (1, 184, 64, 1)
...
def save_config(config: TrainingConfig, output_dir: Path, args, system_info: dict):
    ...
    with open(config_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("XiaoChirp-Accurate with Squeeze-Excitation\n")
        f.write("="*60 + "\n\n")
        ...
        f.write(f"  Model: XiaoChirp-Accurate (Baseline + SE Block)\n")
        f.write(f"  Input Shape: (184, {config.n_mels}, 1)\n")
        f.write(f"  Enhancements: Extra Conv2D + Squeeze-Excitation\n")
        f.write("\n")
        ...
        f.write(f"  Dataset: MyBAD V3\n")
        f.write(f"  Total Samples: 40k (20k positive + 20k negative)\n")
...
def main():
    ...
    config.cache_dir = f'/Volumes/Evo/cache_mybad_fft{config.n_fft}_m{config.n_mels}'
    config.output_dir = f'results/12_accurate_se_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
    ...
    logger.info("="*60)
    logger.info("XiaoChirp-Accurate with Squeeze-Excitation Training")
    logger.info("="*60)
    ...
    try:
        ...
        model = build_xiaochirp_accurate_se(input_shape=(184, config.n_mels, 1), num_classes=2)
        ...
        with open(summary_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("XiaoChirp-Accurate Training Results Summary\n")
            f.write("="*60 + "\n\n")
