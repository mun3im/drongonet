...
- Target: >94.27% accuracy (beat baseline)
...
def build_cnn_mel_high_acc(input_shape=(184, 64, 1), num_classes=2):
    ...
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)  # → (92, 32, 16)

    # Block 2
    x = tf.keras.layers.Conv2D(32, (3, 3), padding='same', activation='relu')(x)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)  # → (46, 16, 32)

    # Block 3 (new)
    x = tf.keras.layers.Conv2D(64, (3, 3), padding='same', activation='relu')(x)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)  # → (23, 8, 64)

    # Global Average Pooling (replaces flatten + small dense)
    ...
    return tf.keras.Model(inputs, outputs, name="MyBAD_HighAcc")
...
def main():
    ...
    config.output_dir = f'results/6_best_accuracy_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
    ...
    try:
        ...
        model = build_cnn_mel_high_acc(input_shape=(184, config.n_mels, 1), num_classes=2)
