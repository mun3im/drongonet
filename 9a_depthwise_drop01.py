#!/usr/bin/env python3
"""
Option 8a: Depthwise Separable + Dropout(0.1)
Ablation Study Model: 8a_depthwise_drop01
Combines parameter efficiency with light regularization
...
def build_cnn_mel_model_table2(
    ...
    """
    CNN-Mel with Depthwise Separable + Dropout(0.1) (Option 8a)
    Combines parameter efficiency with light regularization
    """
    ...
    # MODIFICATION: Add Dropout(0.1)
    x = tf.keras.layers.Dropout(0.1)(x)

    # FC + Softmax -> 2
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_depthwise_d01_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/9a_depthwise_drop01_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
