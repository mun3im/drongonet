#!/usr/bin/env python3
"""
Option I: Depthwise Separable + Dropout(0.4)
Ablation Study Model: 3i_depthwise_drop04
...
def build_cnn_mel_model_table2(
    ...
    """
    CNN-Mel with Depthwise Separable + Dropout(0.4) (Option I)
    Combines parameter efficiency with regularization
    """
    ...
    # MODIFICATION: Add Dropout(0.4)
    x = tf.keras.layers.Dropout(0.4)(x)

    # FC + Softmax -> 2
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_depthwise_d04_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/9d_depthwise_drop04_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
