#!/usr/bin/env python3
"""
Option A: Add Dropout(0.3) before final classifier
Ablation Study Model: 3a_a_dropout
...
def build_cnn_mel_model_table2(
    ...
    Adds Dropout(0.3) before final classifier to reduce overfitting
    """
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_3a_a_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/8c_dropout03_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
