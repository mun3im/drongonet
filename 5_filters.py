#!/usr/bin/env python3
"""
Option B: Double filter count (4→8) in both conv layers
Ablation Study Model: 3a_b_filters
Based on 1a_mybad2_cnnmel.py with specific architectural modification
...
class TrainingConfig:
    ...
    output_dir: str = 'results_3a_b_filters'
...
def build_cnn_mel_model_table2(
    ...
    """
    CNN-Mel + More Filters (Option B)
    Doubles filter count from 4→8 in both conv layers for more capacity
    """
    inputs = tf.keras.layers.Input(shape=input_shape)

    # Block 1: 3x3 valid conv -> ReLU
    # MODIFICATION: 4→8 filters
    x = tf.keras.layers.Conv2D(filters=8, kernel_size=(3, 3), padding="valid")(inputs)
    ...
    # MODIFICATION: 4→8 filters
    x = tf.keras.layers.Conv2D(filters=8, kernel_size=(3, 3), padding="valid")(x)
    x = tf.keras.layers.Activation("relu")(x)
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_5a_filters_m{input_shape[1]}")
...
def main():
    ...
    config.cache_dir = f'/Volumes/Evo/cache_mybad_m{config.n_mels}'
    config.output_dir = f'results/5_filters_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
    ...
    logger.info("="*60)
    logger.info("MyBAD CNN-Mel Training")
    logger.info("="*60)
    ...
    try:
        ...
        with open(summary_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("MyBAD CNN-Mel Training Results Summary\n")
            f.write("="*60 + "\n\n")
