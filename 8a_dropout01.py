#!/usr/bin/env python3
"""
Option A: Add Dropout(0.1) before final classifier
Ablation Study Model: 3a_dropout01
...
class TrainingConfig:
    ...
    output_dir: str = 'results_3a_dropout'
...
def build_cnn_mel_model_table2(
    ...
    """
    CNN-Mel + Dropout (Option A)
    Adds Dropout(0.1) before final classifier to reduce overfitting
    """
    ...
    # MODIFICATION: Add Dropout
    x = tf.keras.layers.Dropout(0.1)(x)

    # FC + Softmax -> 2
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_3a_dropout01_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/8a_dropout01_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
