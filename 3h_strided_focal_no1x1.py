...
class TrainingConfig:
    ...
    output_dir: str = 'results/3h_strided_focal_no1x1'
    ...
    def update_derived_paths(self):
        ...
        self.output_dir = f'results/3h_strided_focal_no1x1_fft{self.n_fft}_m{self.n_mels}_s{self.random_seed}'
...
def build_model(input_shape=(184, 64, 1), num_classes=2):
    ...
    - NO 1x1 conv (removed as redundant) - TIER-1 optimization
    ...
    return tf.keras.Model(inputs, outputs, name="XiaoChirp_7f_strided_focal_no1x1")
...
def verify_model_features(model):
    ...
    # Check that 1x1 convolution is NOT present (we removed it)
    has_1x1_conv = False
    for layer in model.layers:
        if 'conv2d' in layer.__class__.__name__.lower():
            ...
            if kernel_size == (1, 1) or kernel_size == [1, 1]:
                has_1x1_conv = True
                break
    logger.info(f"✓ No redundant 1x1 Conv: {'FAILED - STILL PRESENT' if has_1x1_conv else 'CONFIRMED REMOVED'}")
    assert not has_1x1_conv, "Model should NOT have 1x1 convolution (removed as redundant)"
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("7f: Strided Conv + Focal Loss Tuned + No 1x1 Conv")
    logger.info("=" * 60)
