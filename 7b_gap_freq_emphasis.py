...
class TrainingConfig:
    ...
    n_mels: int = 64  # Using optimal n_mels from baseline sweep  # Number of mel bands
    ...
    output_dir: str = 'results/7b_gap_freq_emphasis'
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train MyBADv2 CNN-Mel model')
...
def get_optimizer(learning_rate: float):
    ...
    Returns:
        TensorFlow optimizer instance (Adam or AdamW)
    """
...
class FrequencyEmphasis(tf.keras.layers.Layer):
    ...
    Adds only ~64 parameters while focusing on bird-relevant frequencies
    """

    def __init__(self, freq_bins=64, init_center=0.4, init_width=0.2, **kwargs):
        ...
        self.weights = self.add_weight(
            ...
            shape=(1, 1, freq_bins, 1),  # [height, width, freq_bins, channels]
        ...
        # Optional: Learn per-frequency gain/attenuation (adds 64 params)
        self.per_freq_gain = self.add_weight(
            name='per_freq_gain',
            shape=(1, 1, freq_bins, 1),
            initializer=tf.keras.initializers.Constant(1.0),
            trainable=False  # Set to True for more expressivity (+64 params)
        )

    def call(self, inputs, training=None):
        # Apply frequency weighting with sigmoid activation
        # Shape: [batch, height, freq_bins, channels] = [batch, 184, 64, 1]
        weighted = inputs * tf.math.sigmoid(self.weights * self.scale)

        # Optionally apply per-frequency gain
...
def build_cnn_gap_freq_emphasis(input_shape=(184, 64, 1), num_classes=2):
    """
    XiaoChirp V1.1 Simple: Just adds frequency emphasis to V1.0
    Expected: ~1,282 + 64 = ~1,346 parameters
    """
    ...
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)  # → (92, 32, 8)
    ...
    model = tf.keras.Model(inputs, outputs, name="XiaoChirp_gap_freq_emphasis")
    return model
...
class ModelTrainer:
    def __init__(self, model: tf.keras.Model, config: TrainingConfig, class_weights: Dict[int, float] = None):
        ...
        self.model.compile(
            ...
            loss=focal_loss(gamma=2.0, alpha=0.5),  # alpha=0.5 for balanced data; try 0.6–0.75 if false negatives dominate
            metrics=[
...
def save_config(config: TrainingConfig, output_dir: Path, args, system_info: dict):
    ...
    with open(config_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("TRAINING CONFIGURATION - MYBADV2 CNN-MEL\n")
        f.write("="*60 + "\n\n")
...
def main():
    ...
    config.cache_dir = f'/Volumes/Evo/cache_mybad_m{config.n_mels}_fft{config.n_fft}/'
    config.output_dir = f'results/7b_gap_freq_emphasis_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
    ...
    logger.info("="*60)
    logger.info("MyBADv2 CNN-Mel Training")
    logger.info("="*60)
    ...
    try:
        ...
        model = build_cnn_gap_freq_emphasis(input_shape=(184, config.n_mels, 1), num_classes=2)
        ...
        with open(summary_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("MyBADv2 CNN-Mel Training Results Summary\n")
            f.write("="*60 + "\n\n")
