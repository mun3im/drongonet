#!/usr/bin/env python3
"""
Dense Layer Ablation - Add Dense(32) before output
...
class TrainingConfig:
    ...
    output_dir: str = 'results/rej_dense32'
...
def parse_args():
    ...
    parser.add_argument('--cache-dir', type=str, default=None,
                        help='Cache directory for mel spectrograms (default: auto-detect based on OS)')
...
def build_cnn_mel_model_table2(
    ...
    """
    Dense(32) Ablation Model
    Adds Dense(32) layer before final output for ablation study
    """
    ...
    # ABLATION: Add Dense(32) layer
    x = tf.keras.layers.Dense(32, activation="relu")(x)

    # FC + Softmax -> 2
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name=f"SEABADNet_rej_dense32_m{input_shape[1]}")
...
class ModelEvaluator:
    ...
    def evaluate_tflite(self, tflite_path: Path, test_dataset: tf.data.Dataset) -> Tuple[float, float, float]:
        ...
        predictions_file = self.output_dir / 'tflite_predictions.npz'
        np.savez_compressed(
            predictions_file,
        ...
        logger.info(f"Saved predictions for threshold analysis to {predictions_file}")
...
def main():
    ...
    # Set cache directory (CLI arg > auto-detect based on OS)
    if args.cache_dir:
        config.cache_dir = args.cache_dir
    else:
        config.cache_dir = f'/Volumes/Evo/cache_seabad_m{config.n_mels}'
    config.output_dir = f'results/rej_dense32_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
    ...
    logger.info("="*60)
    logger.info("MyBAD Dense(32) Ablation Training")
    logger.info("="*60)
