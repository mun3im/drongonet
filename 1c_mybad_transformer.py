#!/usr/bin/env python3
"""
TensorFlow Audio Classification Training Script for MyBAD Dataset
Trains Transformer-Time model on raw audio waveforms
...
class TrainingConfig:
    ...
    output_dir: str = 'results/1c_mybad_transformer'
    cache_dir: str = '/Volumes/Evo/cache_mybad_transformer_waveforms'

def parse_args():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description='Train MyBAD Transformer-Time model')
    ...
    parser.add_argument('--dataset-path', type=str, default='/Volumes/Evo/mybad',
                        help='Path to dataset directory (default: /Volumes/Evo/mybad)')
...
def get_optimizer(learning_rate: float):
    ...
    - Uses legacy Adam optimizer on Apple Silicon (M1/M2/M3/M4) Macs to avoid performance issues.
    - Uses AdamW optimizer on Linux systems.
    - Defaults to standard Adam optimizer otherwise.
    ...
    if is_apple_silicon:
        ...
        logger.info("Detected Linux - using AdamW optimizer")
...
class SingleHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        query = self.query_dense(inputs)  # (batch, time, d_model)
        key = self.key_dense(inputs)      # (batch, time, d_model)
        value = self.value_dense(inputs)  # (batch, time, d_model)

        # Compute attention scores
        score = tf.matmul(query, key, transpose_b=True)  # (batch, time, time)
        score = score / tf.math.sqrt(tf.cast(self.d_model, tf.float32))

        # Apply softmax to get attention weights
        attention_weights = tf.nn.softmax(score, axis=-1)  # (batch, time, time)

        # Apply attention to values
        output = tf.matmul(attention_weights, value)  # (batch, time, d_model)

        return output

def build_transformer_time_model(
    input_shape=(48000, 1),
    num_classes=2
):
    """
    Transformer-Time model for raw waveform input.

    Architecture:
      - Conv1D(filters=16) + ReLU           -> (48000, 16)
      - MaxPooling1D(pool_size=2)            -> (24000, 16)
      - Dropout(0.25)                        -> (24000, 16)
      - MaxPooling1D(pool_size=20)           -> (1200, 16)  # Reduce seq length for transformer
      - AveragePooling1D(pool_size=1200)     -> (1, 16)
      - SingleHeadTransformer                -> (1200, 16)
      - GlobalAveragePooling1D               -> 16
      - Dense(2) + Softmax                   -> 2
    """
    inputs = tf.keras.layers.Input(shape=input_shape)

    # Conv1D + ReLU -> (48000, 16)
    x = tf.keras.layers.Conv1D(filters=16, kernel_size=3, padding="same")(inputs)
    x = tf.keras.layers.Activation("relu")(x)  # (48000, 16)

    # MaxPooling -> (24000, 16)
    x = tf.keras.layers.MaxPooling1D(pool_size=2)(x)  # (24000, 16)

    # Dropout 0.25
    x = tf.keras.layers.Dropout(0.25)(x)  # (24000, 16)

    # Additional pooling to reduce sequence length before transformer
    # This reduces memory requirements from O(24000²) to O(1200²)
    x = tf.keras.layers.MaxPooling1D(pool_size=20)(x)  # (1200, 16)

    # Average Pooling to (1, 16) for positional context
    pooled = tf.keras.layers.AveragePooling1D(pool_size=1200)(x)  # (1, 16)

    # Broadcast pooled features back to (1200, 16) and add to x
    pooled_broadcast = tf.keras.layers.UpSampling1D(size=1200)(pooled)  # (1200, 16)
    x = tf.keras.layers.Add()([x, pooled_broadcast])  # (1200, 16)

    # Single-Head Transformer
    x = SingleHeadAttention(d_model=16)(x)  # (1200, 16)

    # Add layer normalization for stability
    x = tf.keras.layers.LayerNormalization()(x)

    # Global Average Pooling to reduce to (16,)
    x = tf.keras.layers.GlobalAveragePooling1D()(x)  # 16

    # FC + Softmax -> 2
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name="MyBAD_TransformerTime")

    return model

class MyBADv2Dataset:
    """Dataset class for MyBADv2 that returns file paths only, splits data 80:10:10"""
    def __init__(self, root_dir: str, split='train', fraction=1.0, seed=42):
        ...
        # MyBADv2 uses 'positive' and 'negative' folders
        positive_files = []
        negative_files = []
...
def preprocess_and_cache_waveforms(dataset_path: str, config: TrainingConfig, force_reprocess: bool = False):
    ...
    for split in splits:
        ...
        # Get file paths
        dataset = MyBADv2Dataset(dataset_path, split=split, fraction=config.fraction, seed=config.random_seed)
    ...
    def evaluate_tflite(self, tflite_path: Path, test_dataset: tf.data.Dataset) -> Tuple[float, float, float]:
        ...
        for inputs, labels in tqdm(test_dataset, desc="TFLite inference"):
            ...
            for i in range(batch_size):
                input_data = inputs_quantized[i:i + 1]  # (1, 48000, 1)
...
def save_config(config: TrainingConfig, output_dir: Path, args, system_info: dict):
    ...
    with open(config_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("TRAINING CONFIGURATION - TINYCHIRP TRANSFORMER-TIME\n")
        f.write("="*60 + "\n\n")
        ...
        f.write(f"  Model: Transformer-Time\n")
...
def main():
    ...
    config.output_dir = f'results/0c_tinychirp_transformer_r{config.random_seed}_{platform.system().lower()}'
    ...
    logger.info("="*60)
    logger.info("MyBAD Transformer-Time Training")
    logger.info("="*60)
    ...
    try:
        ...
        model = build_transformer_time_model(input_shape=(48000, 1), num_classes=2)
        ...
        with open(summary_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("MyBAD Transformer-Time Training Results Summary\n")
            f.write("="*60 + "\n\n")
