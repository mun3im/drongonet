#!/usr/bin/env python3
"""
Option H: Depthwise Separable + Dropout(0.3)
Ablation Study Model: 3h_depthwise_drop03
...
def build_cnn_mel_model_table2(
    ...
    """
    CNN-Mel with Depthwise Separable + Dropout(0.3) (Option H)
    Combines parameter efficiency with regularization
    """
    ...
    # MODIFICATION: Add Dropout(0.3)
    x = tf.keras.layers.Dropout(0.3)(x)

    # FC + Softmax -> 2
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_depthwise_d03_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/9c_depthwise_drop03_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
