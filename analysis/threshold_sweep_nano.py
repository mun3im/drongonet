#!/usr/bin/env python3
"""
threshold_sweep_nano.py: Threshold sweep for DrongoNet-Nano (3 seeds)

Loads the saved INT8 TFLite model from each of the three validation seeds
(42, 100, 786) of 6a_nano_final_fft1024_m16, runs inference on the
...
Output (written into --out-dir, default: results/nano_threshold_sweep):
...
Usage:
  python threshold_sweep_nano.py \
      --results-base results \
      --cache-dir    /data/cache_seabad_m16 \
      --out-dir      results/nano_threshold_sweep
"""
...
RESULTS_DIRNAME = "6a_nano_final_fft512_m16_s{seed}"
...
def run_tflite_inference(tflite_path: Path, mels: np.ndarray):
    ...
    return np.array(probabilities), float(np.mean(inference_times))
...
def find_locked_tau(mean_rows: list):
    """Highest τ where mean recall ≥ TARGET_RECALL."""
...
def write_per_seed_table(rows: list, out_path: Path, auc: float, seed: int):
    lines = [
        f"DrongoNet-Nano — Threshold Sweep  (seed={seed})",
...
def write_combined_table(all_rows: dict, mean_rows: list, std_rows: list,
    ...
    lines = [
        "DrongoNet-Nano — Combined Threshold Sweep (seeds 42 / 100 / 786)",
        f"Target recall : ≥{TARGET_RECALL}",
...
def write_locked(locked: dict, locked_std: dict, out_path: Path,
    ...
    lines = [
        "DrongoNet-Nano — Locked Threshold",
        "=" * 40,
        f"tau           = {locked['tau']:.2f}",
...
def plot_per_seed_pr(probabilities: np.ndarray, true_labels: np.ndarray,
    ...
    plt.title(f'DrongoNet-Nano — PR curve (INT8, seed={seed})')
...
def plot_combined_pr(all_probs: dict, true_labels: np.ndarray,
    ...
    plt.axvline(x=TARGET_RECALL, color='orange', linestyle=':', lw=1,
                label=f'Target recall ≥{TARGET_RECALL}')
    plt.xlabel('Recall'); plt.ylabel('Precision')
    plt.title('DrongoNet-Nano — PR curves across seeds (INT8 TFLite)')
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(out_path, dpi=150)
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Threshold sweep for DrongoNet-Nano (3 seeds)')
    parser.add_argument('--results-base', type=str, default='results',
                        help='Parent directory containing 6a_nano_final_fft1024_m16_s* dirs')
    parser.add_argument('--cache-dir', type=str, required=True,
                        help='Mel cache dir for n_mels=16 (must contain test/mels.npz)')
    parser.add_argument('--out-dir', type=str, default='results/nano_threshold_sweep',
    ...
    for seed in SEEDS:
        ...
        # Nano uses model_int8.tflite (generated by 6a_nano_final.py)
        tflite_path = result_dir / 'model_int8.tflite'
        if not tflite_path.exists():
            tflite_path = result_dir / 'model.tflite'
        if not tflite_path.exists():
            raise FileNotFoundError(f"TFLite model not found in: {result_dir}")
    ...
    locked_idx = int(np.argmin(np.abs(THRESHOLDS - locked['tau'])))
    ...
    plot_combined_pr(all_probs, true_labels, locked,
                     out_dir / 'threshold_sweep_pr_combined.png')

    print()
    print("=" * 55)
    print("DrongoNet-Nano — locked values (mean ± std, 3 seeds)")
    print("=" * 55)
