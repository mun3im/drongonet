#!/usr/bin/env python3
"""
Option C: Increase dense layer size (8→32 neurons)
Ablation Study Model: 4_dense
...
class TrainingConfig:
    ...
    output_dir: str = 'results_3a_c_dense'
    cache_dir: str = '/Volumes/Evo/cache_mybad2_mels'
...
def build_cnn_mel_model_table2(
    ...
    """
    CNN-Mel + Bigger Dense (Option C)
    Increases dense layer from 8→32 neurons for better feature integration
    """
    ...
    # FC + ReLU -> 32
    # MODIFICATION: 8→32 neurons
    x = tf.keras.layers.Dense(32, activation="relu")(x)

    # FC + Softmax -> 2
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(x)

    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_3a_c_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/4_dense_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}_{platform.system().lower()}'
