#!/usr/bin/env python3
"""
Model 10: Depthwise 6 filters (330K MACs)
...
def build_cnn_mel_model_table2(
    ...
    """
    CNN-Mel with Depthwise Separable Convolutions (Option F)
    Replaces Conv2D with SeparableConv2D for parameter efficiency
    Same architecture otherwise - more efficient than standard convolutions
    """
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_10_depthwise_f6_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/10_depthwise_f6_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
