#!/usr/bin/env python3
"""
Option D: Add BatchNormalization after each conv layer
Ablation Study Model: 3_batchnorm
Based on 1_baseline.py with specific architectural modification
...
class TrainingConfig:
    ...
    output_dir: str = 'results/3_batchnorm'
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train MyBAD CNN-Mel model')
    ...
    parser.add_argument('--dataset-path', type=str, default='/Volumes/Evo/mybad2',
                        help='Path to dataset directory (default: /Volumes/Evo/mybad2)')
...
def build_cnn_mel_model_table2(
    ...
    """
    CNN-Mel + BatchNorm (Option D)
    Adds BatchNormalization after each conv layer for training stability
    """
    inputs = tf.keras.layers.Input(shape=input_shape)

    # Block 1: 3x3 valid conv -> BN -> ReLU
    x = tf.keras.layers.Conv2D(filters=4, kernel_size=(3, 3), padding="valid")(inputs)
    # MODIFICATION: Add BatchNorm
    ...
    # Block 2: 3x3 valid conv -> BN -> ReLU
    ...
    model = tf.keras.models.Model(inputs, outputs, name=f"MyBAD_3a_d_m{input_shape[1]}")
...
def main():
    ...
    config.output_dir = f'results/3_batchnorm_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}_{platform.system().lower()}'
    ...
    logger.info("="*60)
    logger.info("MyBAD XiaoChirp-BatchNorm Training")
    logger.info("="*60)
    ...
    try:
        ...
        model = build_cnn_mel_model_table2(input_shape=(184, config.n_mels, 1), num_classes=2)
