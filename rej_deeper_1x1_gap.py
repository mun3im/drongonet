#!/usr/bin/env python3
"""
14_deeper_gap.py: Deeper GlobalAveragePooling Architecture
3-block CNN with BatchNormalization and GlobalAveragePooling
- 3 Conv blocks (16→32→64 filters)
- BatchNormalization for training stability
- GlobalAveragePooling2D (efficient, no flatten bottleneck)
- Target: >95% accuracy, <25 KB model size
...
class TrainingConfig:
    ...
    output_dir: str = 'results/rej_deeper_1x1_gap'
...
def build_deeper_gap(input_shape=(184, 80, 1), num_classes=2):
    ...
    # 1x1 conv for channel mixing/reduction
    x = tf.keras.layers.Conv2D(16, (1, 1), use_bias=False, name="channel_mix")(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Activation('relu')(x)
    ...
    return tf.keras.Model(inputs, outputs, name="DeeperGAP")  # ← MUST be present
...
def main():
    ...
    config.output_dir = f'results/rej_deeper_1x1_gap_fft{config.n_fft}_m{config.n_mels}_s{config.random_seed}'
